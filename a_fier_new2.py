...
##        predict_song(e1.get(), e2.get(),clf)
            ...
            titlen_vector = pd.Series()
            aname_vector = pd.Series()
            tempo_vector = pd.Series()
            energy_vector = pd.Series()
            loudness_vector = pd.Series()
            danceability_vector = pd.Series()
            valence_vector = pd.Series()
            acousticness_vector = pd.Series()

##            titlen_vector.set_value(titlen)
##            aname_vector.set_value(aname)
##            tempo_vector.set_value(tempo)
##            energy_vector.set_value(energy)
##            loudness_vector.set_value(loudness)
##            danceability_vector.set_value(danceability)
##            valence_vector.set_value(valence)
##            acousticness_vector.set_value(acousticness)
            file_data = [f for f in listdir(path) if isfile (join(path, f))]
            for line in file_data:
                if ( line[-1:] == '\n' ):
                    line = line[:-1]
            
            feature_set['Song Name'] = titlen_vector
            feature_set['Singe'] =aname_vector 
            feature_set['tempo'] =  tempo_vector
            feature_set['energy'] = energy_vector
            feature_set['loudness'] = loudness_vector
            feature_set['danceability'] = danceability_vector
            feature_set['valence'] = valence_vector
            feature_set['acousticness'] = acousticness_vector
##            # 寫入CSV二維表格
##            import csv
##            table = [
##                ['Song Name', 'Singer', 'tempo', 'energy', 'loudness', 'danceability', 'acousticness', 'track_id', 'happy', 'angry', 'sad', 'relax', 'Mood Class', 'Youtube'],
##                [titledata2[i], singerdata2[i], 60]
##            ]
##
...
##                writer.writerows(table)
            ...
            feature_set.to_csv('Emotion_output.csv')
##
##
##    master = Tk()
